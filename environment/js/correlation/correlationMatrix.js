 function correlationMatrix(data2, tempBd){ //cols = tempBd
  d3.select("body").append("div").attr("class", "tip").style("display", "none");

  var corr = jz.arr.correlationMatrix(data2, tempBd);

    var cols = [];

  tempBd.forEach(function(element, index){
    tempCols = tempBd[index].split(":");
    cols.push(tempCols[0]);
  });
  ...
  var dim = d3.min([window.innerWidth * .3, window.innerHeight * .3]);
